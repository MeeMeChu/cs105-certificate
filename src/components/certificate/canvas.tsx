...
import SkeletonTable from "../loading/skelete-table";
...
// URL to your test PDF (in public folder)
const TEST_PDF_URL = "/certificate-template.pdf";
const IMAGE_URL = "/signatures/signature1.png";
...
const Canvas: FC = () => {
  ...
  const handleSelectSignature = (id: number, sigImage: string) => {
    setPositions((prev) =>
      prev.map((p) => {
        if (p.id !== id) {
          return p;
      ...
      <Box sx={{ width: "100%", maxWidth: { sm: "100%", md: "1700px" } }}>
        <Grid container spacing={1}>
          ...
          {upLoadTemplate ? (
            ...
            <Grid size={12}>
              <Alert severity="warning">กรุณา upload certificate template</Alert>
            </Grid>
          )}
        </Grid>
      </Box>
