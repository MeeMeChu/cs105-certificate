...
const loadFont = async (pdfDoc: PDFDocument) => {
  try {
    // Try to load local Thai font
    try {
      console.log("Loading NotoSansThaiLooped-Light font from local file...");
      ...
      console.log("NotoSansThaiLooped-Light font loaded successfully");
      return thaiFont;
    } catch (fontError) {
      console.warn(
        "Local Thai font loading failed:",
        fontError instanceof Error ? fontError.message : String(fontError)
      );

      // Fallback to Times-Roman which supports more characters
      console.log("Using Times-Roman as fallback");
      return await pdfDoc.embedFont("Times-Roman");
    }
  } catch (error) {
    console.error("All font loading failed:", error);
    // Return a basic font that should always work
  ...
  try {
    ...
    if (certificate.templatePath) {
      const templatePath = path.join(
        process.cwd(),
        "public",
        `/uploads/${certificate.templatePath}`
      );
    ...
    for (const position of certificate.positions) {
      if (position.type === "text") {
        ...
        if (textContent) {
          // Use original text without replacement for Thai characters
          ...
          const actualY =
            templateImage.height - position.y * scaleY - textHeight / 2;
        ...
        const actualY =
          templateImage.height - position.y * scaleY - actualHeight / 2;
...
export const generateCertificateForRegistration = async (
  ...
  try {
    ...
    const certificateData: CertificateData = {
      participantName: `${registration.prefix || ""} ${
    ...
    return await generateCertificateWithTemplate(
      targetCertificateId,
      certificateData
    );
  } catch (error) {
...
export const generateCertificatePDF = async (
  ...
  try {
    ...
    // Load font using utility
    const selectedFont = await loadFont(pdfDoc);

    const fontSize = 32;

    // Use original text without replacement
    // คำนวณตำแหน่งของข้อความ (ศูนย์กลาง)
    const textWidth = selectedFont.widthOfTextAtSize(fullName, fontSize);
